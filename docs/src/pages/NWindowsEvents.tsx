...
import M from '../M';
import ClassDescription, { EventDescriptions, EventDescription } from '../ClassDescription';
...
`} >
                        ...
                        <p><M>on_clicked</M> is fired on elements whose <M>clickable</M> property is set to <M>true</M> after an
                            ...
                            when the space or enter key is pressed and a clickable element has keyboard focus. Be aware that 
                            if a mouse button is released outside the current window or outside the window in which the current
                            terminal is being displayed, NWindows will fire an <M>on_mouse_button_released</M> event with a mouse position 
                            of <M>{"{-1,-1}"}</M>. This should be considered a signal to cancel any on-going mouse gestures.
                        </p>
